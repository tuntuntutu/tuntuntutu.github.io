...
## <span id="更新配置">更新配置</span>

SDK 设计为单例模式, 需要更新配置的时候需要调用此接口, 参数格式跟[NIM.getInstance](#初始化SDK)保持一致
...
- `onblacklist`: 同步黑名单的回调, 会传入黑名单列表`blacklist`
    ...
    - 此回调是增量回调, 可以调用[nim.mergeRelations](/doc/web/NIM.html#mergeRelations)和[nim.cutRelations](/doc/web/NIM.html#cutRelations)来合并数据
...
- `onfriends`, 同步好友列表的回调, 会传入[好友](#好友对象)列表`friends`
    ...
    - 此回调是增量回调, 可以调用[nim.mergeFriends](/doc/web/NIM.html#mergeFriends)和[nim.cutFriends](/doc/web/NIM.html#cutFriends)来合并数据
- `onsyncfriendaction`, 当前登录用户在其它端进行好友相关的操作后的回调
    ...
    - 可以调用[nim.mergeFriends](/doc/web/NIM.html#mergeFriends)和[nim.cutFriendsByAccounts](/doc/web/NIM.html#cutFriendsByAccounts)来合并数据
...
- `onusers`: 同步好友用户名片的回调, 会传入[用户名片](#用户名片对象)数组
    - 此回调是增量回调, 可以调用[nim.mergeUsers](/doc/web/NIM.html#mergeUsers)来合并数据
- `onupdateuser`: 用户名片更新后的回调, 会传入[用户名片](#用户名片对象)
...
SDK 提供了普通群以及高级群两种形式的群功能。 高级群拥有更多的权限操作，两种群聊形式在共有操作上保持了接口一致。推荐 APP 开发时只选择一种群类型进行开发。普通群和高级群在原则上是不能互相转换的，他们的群类型在创建时就已经确定。

- 普通群 开发手册中所提及的普通群都等同于 Demo 中的讨论组。普通群没有权限操作，适用于快速创建多人会话的场景。每个普通群只有一个管理员。管理员可以对群进行增减员操作，普通成员只能对群进行增员操作。在添加新成员的时候，并不需要经过对方同意。
- 高级群 高级群在权限上有更多的限制，权限分为群主、管理员、以及群成员。在添加成员的时候需要对方接受邀请。
- 群操作权限对比
...
- `onteams`, 同步[群](#群对象)列表的回调, 会传入群数组
    ...
    - 此回调是增量回调, 可以调用[nim.mergeTeams](/doc/web/NIM.html#mergeTeams)和[nim.cutTeams](/doc/web/NIM.html#cutTeams)来合并数据
- `onsynccreateteam`, 当前登录帐号在其它端创建群之后, 会收到此回调, 会传入[群对象](#群对象)
- `onteammembers`, 同步群成员的回调, 一个群对应一个回调, 会传入群成员数组
    - 可以调用[nim.mergeTeamMembers](/doc/web/NIM.html#mergeTeamMembers)和[nim.cutTeamMembers](/doc/web/NIM.html#cutTeamMembers)来合并数据
...
- `onsessions`, 同步最近会话列表回调, 会传入会话列表, 按时间正序排列, 即最近聊过天的放在列表的最后面
    - 此回调是增量回调, 可以调用[nim.mergeSessions](/doc/web/NIM.html#mergeSessions)来合并数据
...
- 可以调用[nim.mergeMsgs](/doc/web/NIM.html#mergeMsgs)来合并数据
...
- `content`: 自定义消息的消息内容, 开发者可以自行扩展, 建议封装成JSON格式字符串, 请参考[发送自定义消息](#发送自定义消息)
...
- 这里面用到了[nim.cutTeamMembersByAccounts](/doc/web/NIM.html#cutTeamMembersByAccounts)来合并群成员
...
var msg = nim.sendText({
    ...
    text: 'hello',
    custom: '{}',
    // `isHistoryable` 设置成`false`的话, 那么此条消息不会被存储在云端历史
    isHistoryable: false,
    done: sendMsgDone
});
```
...
- 参数`scene`和`to`限制搜索的会话
- 默认查找所有类型的消息, 可以传入参数 `type` 来指定要查找的[消息类型](#消息类型)
- `lastMsgIdClient`为上次查询的最后一条消息的`idClient`, 第一次不填
- `limit`为本次查询的消息数量限制, 最多 100 条, 默认 100 条
- 默认从最近的消息开始往前查找本地消息, 可以传入参数 `reverse=true` 来从第一条消息开始往后查找本地消息
- 默认返回的消息按时间逆序排序, 可以传入参数 `asc=true` 来按时间正序排序
...
#### <span id="获取包含关键词的本地历史记录">获取包含关键词的本地历史记录</span>

- 请参考[获取本地历史记录](#获取本地历史记录)
- 增加了参数`keyword`表示待搜索的关键词, 会匹配消息的`text`字段
- 默认需要通过参数`scene`和`to`指定搜索的会话, 可以传入`global=true`表示搜索所有的会话
...
function searchLocalMsgsDone(error, obj) {
    console.log('搜索本地消息' + (!error?'成功':'失败'));
    console.log(error);
    console.log(obj);
    if (!error) {
...
function deleteLocalMsgDone(error, obj) {
    console.log(error);
    console.log(obj);
    console.log('删除本地消息' + (!error?'成功':'失败'));
}
```
...
- 如果开发者没有 NIM 的实例, 那么需要[从服务器获取聊天室服务器地址](/docs?doc=server&#请求聊天室地址 "target=_blank"), 请参考 demo 来查看具体的做法
...
聊天室设计为单例模式, 需要更新配置的时候需要调用此接口, 参数格式跟[Chatroom.getInstance](#初始化聊天室)保持一致
...
### <span id="退出与重新进入聊天室">退出与重新进入聊天室</span>
...
### <span id="获取当前聊天室的信息">获取当前聊天室的信息</span>
...
- 聊天室通知消息有一个字段`attach`包含了额外的信息, `attach`有一个字段`type`来标识聊天室通知消息的类型
    - `memberEnter`
        - 当有人进入聊天室时, 所有其他聊天室成员会收到类型为`'memberEnter'`的[聊天室通知消息](#聊天室消息)。
    - `memberExit`
        - 当有人退出聊天室时, 所有其他聊天室成员会收到类型为`'memberExit'`的[聊天室通知消息](#聊天室消息)。
    - `addManager`
        - 当有人被[加为管理员](#设置聊天室管理员)时, 所有其他聊天室成员会收到类型为`'addManager'`的[聊天室通知消息](#聊天室消息)。
    - `removeManager`
        - 当有人被[移除管理员](#设置聊天室管理员)时, 所有其他聊天室成员会收到类型为`'removeManager'`的[聊天室通知消息](#聊天室消息)。
    - `addCommon`
        - 当有人被[加为普通成员](#设置聊天室普通成员)时, 所有其他聊天室成员会收到类型为`'addCommon'`的[聊天室通知消息](#聊天室消息)。
    - `removeCommon`
        - 当有人被[移除普通成员](#设置聊天室普通成员)时, 所有其他聊天室成员会收到类型为`'removeCommon'`的[聊天室通知消息](#聊天室消息)。
    - `blackMember`
        - 当有人被[加入黑名单](#设置聊天室黑名单)时, 所有其他聊天室成员会收到类型为`'blackMember'`的[聊天室通知消息](#聊天室消息)。
    - `unblackMember`
        - 当有人被[移除黑名单](#设置聊天室黑名单)时, 所有其他聊天室成员会收到类型为`'blackMember'`的[聊天室通知消息](#聊天室消息)。
    - `gagMember`
        - 当有人被[加入禁言名单](#设置聊天室禁言名单)时, 所有其他聊天室成员会收到类型为`'gagMember'`的[聊天室通知消息](#聊天室消息)。
    - `ungagMember`
        - 当有人被[移除禁言名单](#设置聊天室禁言名单)时, 所有其他聊天室成员会收到类型为`'ungagMember'`的[聊天室通知消息](#聊天室消息)。
    - `kickMember`
        - 当有人被[踢出聊天室](#踢聊天室成员)时, 所有其他聊天室成员会收到类型为`'kickMember'`的[聊天室通知消息](#聊天室消息)。
    - `updateChatroom`
        - 当更新聊天室时, 所有其他聊天室成员会收到类型为`'updateChatroom'`的[聊天室通知消息](#聊天室消息)。
    - `addTempMute`
    - `removeTempMute`
        当有人被[设置聊天室临时禁言](#设置聊天室临时禁言)时，所有其他聊天室成员会收到类型为`'addTempMute' or 'removeTempMute'`的[聊天室通知消息](#聊天室消息)。
...
- 请调用[服务器接口](http://dev.netease.im/docs?doc=server&#修改聊天室开/关闭状态)来关闭聊天室
...
使用[预览文件](#预览文件)和[发送文件消息](#发送文件消息)拿到图片 url 之后，可以调用 SDK 提供的图片操作来处理图片
